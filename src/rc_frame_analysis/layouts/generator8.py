...
def build_active_nodes(
    ...
    if not layout_type or layout_type == "full":
        return nodes
    if layout_type == "layout8":
        # layout8 ????8.png??????????
        if num_spans_x != 7 or num_spans_y != 5:
            raise ValueError("layout8 requires num_spans_x=7 and num_spans_y=5.")
