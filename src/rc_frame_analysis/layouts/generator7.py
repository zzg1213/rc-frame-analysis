...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "layout_type": "layout7",  # ?????"full" ? "layout7"
...
def build_active_nodes(
    ...
    if not layout_type or layout_type == "full":
        return nodes
    if layout_type == "layout7":
        # layout7 ????7.png?????????
        if num_spans_x != 7 or num_spans_y != 6:
            raise ValueError("layout7 requires num_spans_x=7 and num_spans_y=6.")
        j = 3
        for i in (0, 1, 6, 7):
            nodes.discard((i, j))
        return nodes
