...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "layout_type": "layout4",  # ?????"full" ? "layout4"
    "num_spans_range": [7, 7],  # X ????????????
    "num_stories": 8,  # 固定层数（优先于 num_stories_range）
    "num_stories_range": [6, 6],  # 层数随机范围（含端点）
    "num_spans_y_range": [6, 6],  # Y ??????????????
...
def build_active_nodes(
    ...
    if not layout_type or layout_type == "full":
        return nodes
    if layout_type == "layout4":
        # layout4 ????4.png??????
        if num_spans_x != 7 or num_spans_y != 6:
            raise ValueError("layout4 requires num_spans_x=7 and num_spans_y=6.")
        for j in range(4, num_spans_y + 1):
            for i in range(4, num_spans_x + 1):
                nodes.discard((i, j))
        return nodes
...
def build_model(
    ...
    if plane == "XY":
        ...
        even_y_layouts = {"layout4"}
        num_spans_y = cfg.get("num_spans_y")
        if num_spans_y is None:
            if layout_type in even_y_layouts:
                num_spans_y = sample_int(
        ...
        elif num_spans_y % 2 == 0 and layout_type not in even_y_layouts:
            raise ValueError("num_spans_y must be odd.")
        ...
        if not span_lengths_y:
            ...
            if num_spans_y % 2 == 0 and layout_type not in even_y_layouts:
