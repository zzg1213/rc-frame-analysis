...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "layout_type": "layout3",  # 布局类型："full" 或 "layout3"
    "num_spans_range": [7, 7],  # X 向跨数随机范围（含端点）
    "num_stories": 8,  # 固定层数（优先于 num_stories_range）
    "num_stories_range": [6, 6],  # 层数随机范围（含端点）
    "num_spans_y_range": [5, 5],  # Y 向跨数随机范围（自动取奇数）
    "span_length_x_mm": None,  # X 向跨长固定值；None 表示从范围采样并重复
    "span_length_y_mid_mm": 3000.0,  # Y 向中间跨长度（mm）
...
def build_active_nodes(
    ...
    if layout_type == "layout2":
        # layout2 对应布局2.png：顶部与底部中间两列缺失节点
        if num_spans_x != 7 or num_spans_y != 5:
            raise ValueError("layout2 requires num_spans_x=7 and num_spans_y=5.")
        mid_left = num_spans_x // 2
        mid_right = mid_left + 1
        for j in (0, num_spans_y):
            nodes.discard((mid_left, j))
            nodes.discard((mid_right, j))
        return nodes
    if layout_type == "layout3":
        # layout3 对应布局3.png：顶部与底部仅保留左侧三列节点
        if num_spans_x != 7 or num_spans_y != 5:
            raise ValueError("layout3 requires num_spans_x=7 and num_spans_y=5.")
        for j in (0, num_spans_y):
            for i in range(3, num_spans_x + 1):
                nodes.discard((i, j))
        return nodes
...
def build_model(
    ...
    if plane == "XY":
        ...
        if not span_lengths_y:
            ...
            if cfg.get("span_length_y_other_mm") is None:
                span_y_other = sample_float(
                    g,
                    device,
                    y_low,
                    y_high,
                ...
                span_y_other = float(cfg["span_length_y_other_mm"])
            span_y_other = quantize_length(
                span_y_other,
                300.0,
                y_low,
                y_high,
            )
            span_y_mid = quantize_length(
                float(cfg.get("span_length_y_mid_mm", 3900.0)),
                300.0,
                y_low,
                y_high,
            )
            if span_y_mid >= span_y_other:
                span_y_mid = span_y_other - 300.0
            if span_y_mid < y_low:
                span_y_mid = y_low
                if span_y_other <= span_y_mid:
                    span_y_other = span_y_mid + 300.0
                    if span_y_other > y_high:
                        span_y_other = y_high
                        span_y_mid = max(y_low, span_y_other - 300.0)
            span_lengths_y = [span_y_other for _ in range(num_spans_y)]
            mid_idx = num_spans_y // 2
            span_lengths_y[mid_idx] = span_y_mid
        else:
            ...
            if span_lengths_y[num_spans_y // 2] >= max(span_lengths_y):
                raise ValueError("middle span must be smaller than the others.")
