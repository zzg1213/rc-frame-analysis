...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "layout_type": "layout9",  # ?????"full" ? "layout9"
    "num_spans_range": [8, 8],  # X ????????????
    "num_stories": 8,  # 固定层数（优先于 num_stories_range）
    "num_stories_range": [6, 6],  # 层数随机范围（含端点）
    "num_spans_y_range": [4, 4],  # Y ??????????????
...
def build_active_nodes(
    ...
    if not layout_type or layout_type == "full":
        return nodes
    if layout_type == "layout9":
        # layout9 对应布局9.png：右下角阶梯缺口（忽略图内开洞）
        if num_spans_x != 8 or num_spans_y != 4:
            raise ValueError("layout9 requires num_spans_x=8 and num_spans_y=4.")
        # 底行：i=4..8；次底行：i=6..8（j=0 为底部）
        for i in range(4, 9):
            nodes.discard((i, 0))
        for i in range(6, 9):
            nodes.discard((i, 1))
